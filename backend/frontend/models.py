...
class Review(models.Model):
    ...
    id = models.CharField(max_length=22, default=shortuuid.uuid, unique=True, primary_key=True, editable=False)
    orderProduct = models.ForeignKey(OrderProduct, on_delete=models.CASCADE, null=False, blank=False, related_name='order_product_reviews')
    body = models.TextField(max_length=2000, null=True, blank=True)
    rating = models.IntegerField(choices=RATING_CHOICES, null=True, blank=True)
    created = models.DateTimeField(auto_now_add=True)

    def __str__(self):
         return str(self.orderProduct.product)
